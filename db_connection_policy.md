...
## 9) Oracle driver 의존성 정책

- 실DB 수집은 Python `oracledb` 드라이버를 사용한다.
- 기본 설치(`pip install -e .`)에는 Oracle 드라이버를 강제하지 않는다.
- Oracle live collector 실행 환경은 선택 의존성 `pip install -e .[oracle]` 또는 별도 `pip install oracledb`로 드라이버를 설치해야 한다.
- 드라이버가 없으면 `lab collect db`는 DB 접속을 시도하지 않고 FAIL_FAST로 종료하며, 비밀번호/DSN 원문을 출력하지 않는다.
- 테스트/fixture 환경은 코드에서 명확히 주입한 fake driver만 사용하고, 운영 경로의 `collection_mode`는 `live_query`로 기록한다.
