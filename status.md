...
#### 2026-05-09 00:00
- Scope: NEXT-07 `db_collection.json` 기반 DB 명세서 Markdown smoke path 보완.
- Completed: 기존 `lab generate db-schema` 흐름을 재사용해 `db_collection.json` 입력에서 `db_schema.json` 및 Markdown 명세서를 생성하는 경로를 보강함; Markdown에 `generated_at`, 컬럼 기본값, 인덱스 섹션/카운트를 추가함; fixture 내 index 정보를 정규화/검증 schema에 반영함; 요청 산출물 경로 `artifacts/docs/local-oracle/db_spec.md` 실행 예시를 문서화함; smoke test에서 해당 local-oracle 문서 경로 형태로 파일 생성 및 table/column 내용 포함을 검증함.
- Files changed: `src/lab/db/normalizer.py`, `src/lab/db/renderer.py`, `src/lab/quality/validate_db.py`, `db_schema.schema.json`, `tests/test_w6_db_schema_smoke.py`, `docs/CLI.md`, `status.md`.
- Validation: PASS `PYTHONPATH=src pytest -q tests/test_w6_db_schema_smoke.py -q` (11 passed); PASS `PYTHONPATH=src pytest -q` (37 passed); PASS `python -m py_compile src/lab/db/normalizer.py src/lab/db/renderer.py src/lab/quality/validate_db.py src/lab/commands/generate_db_schema.py`; PASS `git diff --check`; PASS fixture smoke command `tmpdir=$(mktemp -d) && PYTHONPATH=src python -m lab generate db-schema --input tests/fixtures/db/sample_db_input.json --json-output "$tmpdir/artifacts/db/local-oracle/db_schema.json" --output "$tmpdir/artifacts/docs/local-oracle/db_spec.md" && test -s "$tmpdir/artifacts/docs/local-oracle/db_spec.md" && rg -n "PUBLIC\.USERS|\| 1 \| ID \| BIGINT" "$tmpdir/artifacts/docs/local-oracle/db_spec.md"`; WARN live artifact command `PYTHONPATH=src python -m lab generate db-schema --input artifacts/db/local-oracle/db_collection.json --json-output artifacts/db/local-oracle/db_schema.json --output artifacts/docs/local-oracle/db_spec.md` skipped because `artifacts/db/local-oracle/db_collection.json` is not present in this checkout.
